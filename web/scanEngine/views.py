    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return http.HttpResponseRedirect(
                reverse("scan_engine_index", kwargs={"slug": slug})
            )
    ...
    if request.method == "POST":
        if form.is_valid() and "upload_file" in request.FILES:
            ...
            if txt_file.content_type == "text/plain":
                ...
                return http.HttpResponseRedirect(
                    reverse("wordlist_list", kwargs={"slug": slug})
                )
    context["form"] = form
...
def delete_wordlist(request, slug, id):
    ...
    if request.method == "POST":
        obj.delete()
        try:
            os.remove("/usr/src/" + "wordlist/" + obj.short_name + ".txt")
            responseData = {"status": True}
...
def interesting_lookup(request, slug):
    ...
    if InterestingLookupModel.objects.filter(custom_type=True).exists():
        lookup_keywords = InterestingLookupModel.objects.filter(
            custom_type=True
        ).order_by("-id")[0]
    else:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.add_message(
                request, messages.INFO, "Lookup Keywords updated successfully"
            )
            return http.HttpResponseRedirect(
                reverse("interesting_lookup", kwargs={"slug": slug})
            )
    ...
    if request.method == "POST":
        ...
        if "gfFileUpload[]" in request.FILES:
            ...
            for gf_file in gf_files:
                original_filename = (
                    gf_file.name
                    if isinstance(gf_file.name, str)
                    else gf_file.name.decode("utf-8")
                )
                ...
                if file_extension == "json":
                    ...
                    upload_count += 1
            messages.add_message(
                request, messages.INFO, f"{upload_count} GF files successfully uploaded"
            )
            return http.HttpResponseRedirect(
                reverse("tool_settings", kwargs={"slug": slug})
            )
            ...
            for nuclei_file in nuclei_files:
                original_filename = (
                    nuclei_file.name
                    if isinstance(nuclei_file.name, str)
                    else nuclei_file.name.decode("utf-8")
                )
                original_filename = re.sub(r'[\\/*?:"<>|]', "", original_filename)
                file_extension = original_filename.split(".")[
            ...
            messages.add_message(
                request,
                messages.INFO,
                f"{upload_count} Nuclei Patterns successfully uploaded",
            )
            return http.HttpResponseRedirect(
                reverse("tool_settings", kwargs={"slug": slug})
            )
...
def notification_settings(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return http.HttpResponseRedirect(
                reverse("notification_settings", kwargs={"slug": slug})
            )
...
def proxy_settings(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.add_message(request, messages.INFO, "Proxies updated.")
            return http.HttpResponseRedirect(
                reverse("proxy_settings", kwargs={"slug": slug})
            )
    context["settings_nav_active"] = "active"
...
def hackerone_settings(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.add_message(request, messages.INFO, "Hackerone Settings updated.")
            return http.HttpResponseRedirect(
                reverse("hackerone_settings", kwargs={"slug": slug})
            )
    context["settings_nav_active"] = "active"
...
def report_settings(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.add_message(request, messages.INFO, "Report Settings updated.")
            return http.HttpResponseRedirect(
                reverse("report_settings", kwargs={"slug": slug})
            )
            ...
            all_models.append(
                {
                    **model,
                    "modified_at": datetime.strptime(
                        model["modified_at"].split(".")[0], date_format
                    ),
                    "is_local": True,
                }
            )
        ...
        if key_hackerone and username_hackerone:
            ...
            if hackerone_api_key:
                ...
                HackerOneAPIKey.objects.create(
                    username=username_hackerone, key=key_hackerone
                )
...
def add_tool(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return http.HttpResponseRedirect(
                reverse("tool_arsenal", kwargs={"slug": slug})
            )
