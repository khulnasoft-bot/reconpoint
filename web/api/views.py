...
from api.shared_api_tasks import (
    import_hackerone_programs_task,
    sync_bookmarked_programs_task,
)
...
class HackerOneProgramViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        try:
            ...
            if sort_by == "name":
                ...
                programs = sorted(
                    programs,
                    key=lambda x: x["attributes"].get("number_of_reports_for_user", 0),
                    reverse=(sort_order.lower() == "desc"),
                )
            elif sort_by == "age":
                programs = sorted(
                    ...
                    key=lambda x: datetime.strptime(
                        x["attributes"].get(
                            "started_accepting_at", "1970-01-01T00:00:00.000Z"
                        ),
                        "%Y-%m-%dT%H:%M:%S.%fZ",
                    ),
                    reverse=(sort_order.lower() == "desc"),
    ...
    def bounty_programs(self, request):
        try:
            programs = self.get_cached_programs()
            bounty_programs = [
                p for p in programs if p["attributes"]["offers_bounties"]
            ]
    ...
    def fetch_programs_from_hackerone(self):
        ...
        while url:
            ...
            if response.status_code == 401:
                ...
                raise Exception(
                    f"HackerOne API request failed with status code {response.status_code}"
                )
    ...
    def program_details(self, request, pk=None):
        try:
            ...
            if program_details is None:
                program_details = self.fetch_program_details_from_hackerone(
                    program_handle
                )
                if program_details:
            ...
            if program_details:
                filtered_scopes = [
                    scope
                    for scope in program_details.get("relationships", {})
                    .get("structured_scopes", {})
                    .get("data", [])
                    if scope.get("attributes", {}).get("asset_type")
                    in self.ALLOWED_ASSET_TYPES
                ]

                program_details["relationships"]["structured_scopes"][
                    "data"
                ] = filtered_scopes
    ...
    def import_programs(self, request):
        try:
            ...
            if not handles:
                return Response(
                    {"error": "No program handles provided"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                {"message": f"Import process for {len(handles)} program(s) has begun."},
    ...
    def sync_bookmarked(self, request):
        try:
            ...
            return Response(
                {"message": "Sync process for bookmarked programs has begun."},
                status=status.HTTP_202_ACCEPTED,
            )
        except Exception as e:
            return self.handle_exception(e)
    ...
    def handle_exception(exc):
        if isinstance(exc, ObjectDoesNotExist):
            ...
            return Response(
                {"error": "Invalid HackerOne API credentials"},
    ...
    def get(self, request):
        ...
        try:
            ...
            _response = requests.post(
                pull_model_api, json={"name": model_name, "stream": False}
            ).json()
    ...
    def delete(self, request):
        ...
        try:
            _response = requests.delete(
                delete_model_api, json={"name": model_name}
            ).json()
            if _response.get("error"):
                response["status"] = False
                response["error"] = _response.get("error")
    ...
    def put(self, request):
        ...
        try:
            OllamaSettings.objects.update_or_create(
                defaults={"selected_model": model_name, "use_ollama": use_ollama}, id=1
            )
            response["status"] = True
        except Exception as e:
            response["error"] = str(e)
        ...
        def abort_subscan(subscan):
            ...
            try:
                ...
                create_scan_activity(
                    subscan.scan_history.id, f"Subscan aborted", ABORTED_TASK
                )
                response["status"] = True
            except Exception as e:
                logger.error(e)
...
class ReconpointUpdateCheck(APIView):
    def get(self, request):
        ...
        is_version_update_available = version.parse(current_version) < version.parse(
            latest_version
        )
    ...
    def get(self, request):
        ...
        try:
            ...
            for ip in IPv4Network(ip_address, False):
                ...
                resolved_ips.append(
                    {"ip": str(ip), "domain": domain, "domains": domains, "ips": ips}
                )
            response = {
                "status": True,
                "orig": ip_address,
