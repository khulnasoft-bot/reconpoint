...
class ScanFindingMetricsTestCase(BaseTestCase):
    ...
    def test_partition_ip_address_ids_for_target(self) -> None:
        ...
        sub.ip_addresses.add(ip_in)
        valid, invalid = partition_ip_address_ids_for_target(
            [ip_in.id, ip_out.id], target.id
        )
    ...
    def test_bulk_ip_metrics_for_scans(self) -> None:
        ...
        sub_b = self.data_generator.create_subdomain(
            scan_history=scan_b, domain=domain_b
        )
    ...
    def test_bulk_ip_metrics_for_targets_matches_union_across_scans(self) -> None:
        ...
        scan_a = self.scan
        sub_a = self.data_generator.create_subdomain(
            scan_history=scan_a, domain=self.domain
        )
    ...
    def test_ip_address_get_project_counts_matches_service(self) -> None:
        ...
        ip_dead = IpAddress.objects.create(
            address="192.0.2.100", version=4, alive=False
        )
        ip_alive = IpAddress.objects.create(
            address="192.0.2.101", version=4, alive=True
        )
    ...
    def test_light_websocket_payload_ip_counts_match_get_scan_finding_counts(
        ...
        self.assertEqual(
            light[SCAN_FINDING_IP_COUNT_KEY], counts[SCAN_FINDING_IP_COUNT_KEY]
        )
        self.assertEqual(
            light[SCAN_FINDING_IP_ALIVE_KEY], counts[SCAN_FINDING_IP_ALIVE_KEY]
        )
