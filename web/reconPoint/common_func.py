...
def get_lookup_keywords():
    ...
    if lookup_model:
        default_lookup_keywords = [
            key.strip() for key in lookup_model.keywords.split(",")
        ]
    if lookup_obj:
        custom_lookup_keywords = [key.strip() for key in lookup_obj.keywords.split(",")]
    lookup_keywords = default_lookup_keywords + custom_lookup_keywords
    lookup_keywords = list(
        filter(None, lookup_keywords)
...
def send_discord_message(
    message, title="", severity=None, url=None, files=None, fields={}, fields_append=[]
):
...
def get_task_cache_key(func_name, *args, **kwargs):
    ...
    kwargs_str = "_".join(
        [
            ...
            if k not in RECONPOINT_TASK_IGNORE_CACHE_KWARGS
        ]
    )
    ...
    try:
        for section in sections:
            ...
            if section_title == "Description":
                ...
                data["references"] = [
                    ref.strip() for ref in content.split("\n") if ref.strip()
                ]
    except Exception as e:
        return data
...
def update_or_create_port(port_number, service_name=None, description=None):
    ...
    try:
        ...
        port = Port.objects.create(
            number=port_number, service_name=service_name, description=description
        )
        created = True
    finally:
        return port, created
...
def extract_domain_info(domain):
    ...
    try:
        ...
        domain_info.update(
            {
                "related_tlds": [
                    domain.name for domain in domain_info_db.related_tlds.all()
                ],
                "related_domains": [
                    domain.name for domain in domain_info_db.related_domains.all()
                ],
            }
        )
...
def parse_registration_info(domain_info, registration, role):
    ...
    if email:
        ...
        domain_info[f"{role_prefix}_email"] = (
...
def save_domain_info_to_db(target, domain_info):
    ...
    if Domain.objects.filter(name=target).exists():
        ...
        for related_domain in domain_info.get("related_domains", []):
            related_domain_obj, _ = RelatedDomain.objects.get_or_create(
                name=related_domain
            )
