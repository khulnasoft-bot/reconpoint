...
def get_secator_prefix_diagnostic(sample_size: int = 20) -> dict:
    ...
    reconpoint_results = getattr(
        settings, "RECONPOINT_RESULTS", str(Path.home() / "scan_results")
    )
    base = Path(reconpoint_results).resolve()
    reconpoint_results_exists = base.exists()
    reconpoint_results_readable = (
        reconpoint_results_exists and base.is_dir() and os.access(base, os.R_OK)
    )
    ...
    ep_screenshot = EndPoint.objects.filter(screenshot_path__isnull=False).exclude(
        screenshot_path=""
    )
    ep_stored = EndPoint.objects.filter(stored_response_path__isnull=False).exclude(
        stored_response_path=""
    )
    tech_stored = Technology.objects.filter(stored_response_path__isnull=False).exclude(
        stored_response_path=""
    )
