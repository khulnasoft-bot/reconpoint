...
class ReconpointTask(Task):
    ...
    def __call__(self, *args, **kwargs):
        ...
        self.description = (
            ...
            or " ".join(self.task_name.split("_")).capitalize()
        )
        ...
        if not self.filename:
            self.filename = get_output_file_name(
                self.scan_id, self.subscan_id, f"{self.task_name}.txt"
            )
        ...
        if RECONPOINT_RECORD_ENABLED:
            if self.engine:  # task not in engine.tasks, skip it.
                ...
                if (
                    self.track
                    and self.task_name not in self.engine.tasks
                    and dependent_tasks.get(self.task_name) not in self.engine.tasks
                ):
                    logger.debug(
                        f'Task {self.name} is not part of engine "{self.engine.engine_name}" tasks. Skipping.'
                    )
                    return
        ...
        try:
            ...
            self.output_path = get_traceback_path(
                self.task_name, self.results_dir, self.scan_id, self.subscan_id
            )
