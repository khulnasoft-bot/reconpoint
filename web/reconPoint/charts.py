...
def generate_subdomain_chart_by_http_status(subdomains):
    ...
    http_status_count = [
        {"http_status": entry["http_status"], "count": entry["count"]}
        for entry in http_statuses
    ]
    ...
    colors = [
        get_color_by_http_status(entry["http_status"]) for entry in http_status_count
    ]

    text = [
        f"{label}<br>{size}<br>({size/total:.1%})" for label, size in zip(labels, sizes)
    ]
    ...
    fig.update_layout(
        title_text="",
        annotations=[
            dict(text="HTTP Status", x=0.5, y=0.5, font_size=14, showarrow=False)
        ],
        showlegend=True,
...
def get_color_by_severity(severity_int):
    ...
    return color_map.get(
        severity_int, "#A8A9AD"
    )  # Default to gray if severity is unknown
...
def generate_vulnerability_chart_by_severity(vulnerabilities):
    ...
    labels = [
        NUCLEI_REVERSE_SEVERITY_MAP[entry["severity"]].capitalize()
    ...
    text = [
        f"{label}<br>{value}<br>({value/total:.1%})"
    ...
    fig.update_layout(
        title_text="",
        annotations=[
            dict(text="Severity", x=0.5, y=0.5, font_size=14, showarrow=False)
        ],
        showlegend=True,
        margin=dict(t=60, b=60, l=60, r=60),
        width=700,
