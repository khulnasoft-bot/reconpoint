...
class LLMVulnerabilityReportGenerator:
    ...
    def get_vulnerability_description(self, description):
        ...
        if self.use_ollama:
            ...
            self.logger.info(
                f"Using OpenAI API for Vulnerability Description Generation"
            )
            openai_api_key = get_open_ai_key()
            if not openai_api_key:
                return {"status": False, "error": "OpenAI API Key not set"}
            try:
    ...
    def get_attack_suggestion(self, user_input):
        ...
        if self.use_ollama:
            ...
            try:
                ...
                return {"status": False, "error": str(e), "input": user_input}
        return {"status": True, "description": response_content, "input": user_input}
...
class ReconAgent:
    def __init__(self, logger):
        ...
        if self.use_ollama:
            ...
            openai_api_key = get_open_ai_key()
            self.llm = ChatOpenAI(
                model_name=self.model_name, openai_api_key=openai_api_key
            )
