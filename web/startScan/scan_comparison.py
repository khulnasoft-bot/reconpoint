...
class ScanDiffer:
    ...
    def diff_subdomains(self) -> DiffResult:
        ...
        old_subdomains = {
            s.name: s
            for s in Subdomain.objects.filter(scan_history=self.old_scan)
        }
        new_subdomains = {
            s.name: s
            for s in Subdomain.objects.filter(scan_history=self.new_scan)
        }
    ...
    def diff_endpoints(self) -> DiffResult:
        ...
        old_endpoints = {
            e.http_url: e
            for e in EndPoint.objects.filter(scan_history=self.old_scan)
        }
        new_endpoints = {
            e.http_url: e
            for e in EndPoint.objects.filter(scan_history=self.new_scan)
        }
    ...
    def diff_vulnerabilities(self) -> DiffResult:
        ...
        old_vulns = {
            (v.name, v.severity): v
            for v in Vulnerability.objects.filter(scan_history=self.old_scan)
        }
        new_vulns = {
            (v.name, v.severity): v
            for v in Vulnerability.objects.filter(scan_history=self.new_scan)
        }
    ...
    def diff_technologies(self) -> DiffResult:
        ...
        old_techs = {
            t.name: t
            for t in Technology.objects.filter(scan_history=self.old_scan)
        }
        new_techs = {
            t.name: t
            for t in Technology.objects.filter(scan_history=self.new_scan)
        }
        ...
        added = [
            {"name": name, "version": new_techs[name].version}
            for name in added_names
        ]
        removed = [
            {"name": name, "version": old_techs[name].version}
...
class SecurityPostureTracker:
    ...
    def get_posture_history(
        ...
        ).order_by("start_scan_date").select_related("scan_type")
    ...
    def calculate_posture_score(self, scan: ScanHistory) -> float:
        ...
        penalty = sum(
            vuln_counts.get(severity, 0) * weight
...
class ScanComparisonReport:
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        suspicious_changes = new_tracker.detect_suspicious_changes(
            self.old_scan, self.new_scan
        )
        ...
        return {
            ...
            "change_summary": self._generate_change_summary(
                subdomain_diff, endpoint_diff, vuln_diff, tech_diff
            ),
        }
    ...
    def _generate_change_summary(
        ...
        total_removed = subdomains.stats["removed_count"] + endpoints.stats["removed_count"] + vulns.stats["removed_count"]
