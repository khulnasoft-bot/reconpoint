...
def detail_scan(request, id, slug):
    ...
    alive_count = (
        subdomains.values("name").distinct().filter(http_status__exact=200).count()
    )
    important_count = (
    ...
    endpoint_alive_count = (
        ...
        )  # TODO: use is_alive() func as it's more precise
        .values("http_url")
        .distinct()
        .count()
    )
    ...
    severity_counts = vulns.aggregate(
        info_count=Count("id", filter=models.Q(severity=0)),
        low_count=Count("id", filter=models.Q(severity=1)),
        medium_count=Count("id", filter=models.Q(severity=2)),
        high_count=Count("id", filter=models.Q(severity=3)),
        critical_count=Count("id", filter=models.Q(severity=4)),
        unknown_count=Count("id", filter=models.Q(severity=-1)),
    )
    total_count = vulns.count()
    total_count_ignore_info = total_count - severity_counts["info_count"]
    ...
    ctx = {
        ...
        "vulnerability_list": vulns.order_by("-severity")
        .prefetch_related("tags", "cve_ids", "cwe_ids")
        .all(),
...
def all_subdomains(request, slug):
    ...
    counts = subdomains.aggregate(
        subdomain_count=Count("name", distinct=True),
        alive_count=Count("name", distinct=True, filter=models.Q(http_status=200)),
        important_count=Count("name", distinct=True, filter=models.Q(is_important=True)),
    )
...
def start_scan_ui(request, slug, domain_id):
    ...
    custom_engines_count = EngineType.objects.filter(default_engine=False).count()
    excluded_paths = (
        ",".join(DEFAULT_EXCLUDED_PATHS)
        if not excluded_paths
        else ",".join(excluded_paths)
    )
    ...
    if request.method == "POST":
        for key, value in request.POST.items():
            ...
            try:
                ...
                create_scan_activity(scan.id, "Scan aborted", ABORTED_TASK)
                messages.add_message(
                    request, messages.INFO, "Multiple scans successfully stopped!"
                )
            except Exception as e:
                logger.error(e)
                messages.add_message(
                    request, messages.ERROR, f"Scans failed to stop ! Error: {str(e)}"
                )
    return HttpResponseRedirect(reverse("scan_history", kwargs={"slug": slug}))
    ...
    if request.method == "POST":
        ...
        if scheduled_mode == "periodic":
            ...
            schedule, _ = IntervalSchedule.objects.get_or_create(
                every=frequency_value, period=period
            )
            kwargs = {
                "domain_id": host_id,
                "engine_id": engine.id,
        ...
        messages.add_message(
            request, messages.INFO, f"Scan Scheduled for {domain.name}"
        )
    ...
    if request.method == "POST":
        for key, value in request.POST.items():
            ...
            try:
                ...
                logger.error(e)
        messages.add_message(
            request, messages.INFO, "Multiple scheduled scans successfully deleted!"
        )
        return HttpResponseRedirect(
            reverse("scheduled_scan_view", kwargs={"slug": slug})
        )
...
def delete_scans(request, slug):
    if request.method == "POST":
        for key, value in request.POST.items():
            ...
            scan.delete()
        messages.add_message(
            request, messages.INFO, "Multiple scans successfully deleted!"
        )
    return HttpResponseRedirect(reverse("scan_history", kwargs={"slug": slug}))
...
def create_report(request, id):
    ...
    report_template = (
        request.GET["report_template"]
        if "report_template" in request.GET
        else "default"
    )
    ...
    if vuln_report_query.exists():
        ...
        if scan.domain.description:
            description = description.replace(
                "{target_description}", scan.domain.description
            )
    ...
    data["subdomain_http_status_chart"] = generate_subdomain_chart_by_http_status(
        subdomains
    )
    data["vulns_severity_chart"] = (
        generate_vulnerability_chart_by_severity(vulns) if vulns else ""
    )
