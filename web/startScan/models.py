...
from reconPoint.definitions import CELERY_TASK_STATUSES, NUCLEI_REVERSE_SEVERITY_MAP
...
class Subdomain(models.Model):
    ...
    def get_vulnerabilities_without_info(self):
        vulns = Vulnerability.objects.filter(subdomain__name=self.name).exclude(
            severity=0
        )
        if self.scan_history:
            vulns = vulns.filter(scan_history=self.scan_history)
...
class SubScan(models.Model):
    ...
    class Meta:
        indexes = [
            models.Index(fields=["scan_history"]),
...
class GPTVulnerabilityReport(models.Model):
    ...
    references = models.ManyToManyField(
        "VulnerabilityReference", related_name="report_reference", blank=True
    )
