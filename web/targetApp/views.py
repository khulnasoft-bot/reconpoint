    ...
    if request.method == "POST":
        ...
        try:
            ...
            # Import from txt / csv
                    ...
                    for target in io_string:
                        ...
                        if not domain_query.exists():
                            if not validators.domain(
                                target_domain
                            ):  # Change 'domain' to 'target_domain'
                                messages.add_message(
                                    request,
                                    messages.ERROR,
                ...
                resolved_ips = [
                    ip.rstrip()
                    for ip in request.POST.getlist("resolved_ip_domains")
                    if ip
                ]
...
def target_summary(request, slug, id):
    ...
    ignore_info_count = sum(
        [
            severity_counts["low_count"],
            severity_counts["medium_count"],
            severity_counts["high_count"],
            severity_counts["critical_count"],
        ]
    )
    context.update(severity_counts)
