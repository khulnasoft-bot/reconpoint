...
def build_scan_params_form_context(
    ...
    if scan_params_values is not None:
        ...
        target is not None
        and getattr(target, "scan_config", None)
        and isinstance(target.scan_config, dict)
    ):
        ...
        values = (
            dict(organization.scan_config)
            if isinstance(organization.scan_config, dict)
            else {}
        )
    else:
        values = {}
    ...
    if (
        ...
        scope
        and getattr(scope, "scan_config", None)
        and isinstance(scope.scan_config, dict)
    ):
        entity_config = scope.scan_config
    elif (
        organization
        and getattr(organization, "scan_config", None)
        and isinstance(organization.scan_config, dict)
    ):
        entity_config = organization.scan_config
    if entity_config:
        profiles = entity_config.get("profiles")
        if profiles and isinstance(profiles, dict):
            profiles_ctx["default_profiles"] = {
                cat: (profiles.get(cat) or "") for cat in PROFILE_CATEGORIES
            }

    values.setdefault("profiles", {})
    raw_profiles = values.get("profiles")
    values["profiles"] = {
        cat: ((raw_profiles.get(cat) if isinstance(raw_profiles, dict) else "") or "")
        for cat in PROFILE_CATEGORIES
    }
    entity_profile_categories = [
        cat for cat in PROFILE_CATEGORIES if values["profiles"].get(cat, "").strip()
    ]
    ...
    has_overrides = any(
        ...
        if k != "profiles" and not (isinstance(v, dict) and not v)
    ) or any(
        ((values.get("profiles") or {}).get(c) or "").strip()
    ...
    if level == "target":
        ...
        section_help_text = "Leave fields empty to inherit from the level above or system defaults. Filled values apply at this level."

    scan_params_effective_ordered = [
        (p, effective[p]) for p in ORDERED_PARAM_KEYS_FOR_FORM if p in effective
    ]
    result = {
        ...
        "scan_params_section_configure_button_label": "Configured"
        if has_overrides
        else "Configure",
        **profiles_ctx,
    }
    effective_scope = (
        ...
        else (get_scope_for_target(target) if target else None)
    )
    if effective_scope is not None:
        result["scan_params_allow_local_worker"] = scope_allow_local(effective_scope)
        result["scan_params_default_worker_id"] = get_default_worker_for_scope(
            effective_scope
        )
    return result
