"""Runtime abstraction for ReconAgent."""
...
class LocalRuntime(Runtime):
    ...
    async def start(self):
        ...
        self._running = True
        # Create organized loot directory structure in the project root
        project_root = Path(__file__).parent.parent.parent.parent.parent # This is a bit brittle, but assuming standard Django layout
        loot_dir = project_root / "loot"
        loot_dir.mkdir(exist_ok=True)
        (loot_dir / "reports").mkdir(exist_ok=True)
        (loot_dir / "artifacts").mkdir(exist_ok=True)
        (loot_dir / "artifacts" / "screenshots").mkdir(exist_ok=True)
    ...
    async def _execute_browser_action(self, action: str, **kwargs) -> dict:
        ...
        try:
            if action == "navigate":
                ...
                output_dir = project_root / "loot" / "artifacts" / "screenshots"
