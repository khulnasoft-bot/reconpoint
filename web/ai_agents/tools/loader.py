"""Dynamic tool loader for ReconAgent."""
...
def load_tool_module(module_name: str, tools_dir: Optional[Path] = None) -> bool:
    ...
    try:
        ...
        full_module_name = f"web.ai_agents.tools.{module_name}"
...
def reload_tools():
    ...
    to_remove = [
        ...
        if name.startswith("web.ai_agents.tools.")
        and name
        not in (
            "web.ai_agents.tools",
            "web.ai_agents.tools.registry",
            "web.ai_agents.tools.executor",
            "web.ai_agents.tools.loader",
        )
    ]
